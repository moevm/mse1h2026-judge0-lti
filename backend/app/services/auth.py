...
class AuthService:
    ...
    async def login(self, body: AuthRequest):
    ...
    async def issue_lti_session(self, user):
        await self.token_repo.delete_all_by_user(user.id)
    ...
    async def get_user_from_refresh(self, refresh_token: str) -> User:
        ...
        if not db_token or db_token.revoked:
            raise InvalidRefreshTokenException()
        return await self.user_repo.get_by_id(db_token.user_id)
