...
class SolutionService:
    ...
    async def get_solutions_by_task(self, task_id: int, filters: SolutionFilter) -> List[Solution]:
        return await self.repo.get_by_task_id_with_filters(task_id, filters)

    async def get_solution_attempts(self, solution_id: int) -> List[Attempt]:
        solution = await self.repo.get_by_id(solution_id)
        if not solution:
            raise TaskNotFoundException(f"Решение {solution_id} не найдено")
        return await self.attempt_repo.get_by_solution_id(solution_id)
